...
export function PortalShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="portal-frame">
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav" aria-label="Primary navigation">{links.filter(([href], index) => user.role === "admin" || (index !== 3 && href !== "/teams")).map(([href, label, icon]) => <Link key={href} href={href} className={pathname === href ? "active" : ""}><span className="nav-icon">{icon}</span>{label}</Link>)}</nav>
