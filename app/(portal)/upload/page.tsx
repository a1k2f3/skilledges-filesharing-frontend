...
export default function UploadPage() {
  ...
  return <><PageHeader eyebrow="Production intake" title="Upload a new design" description="Send artwork and stitch requirements to the digitizing team." /><section className="panel form-panel"><div className="form-grid"><div>{user.role === "admin" && <label>Customer<select value={customer} onChange={(event) => setCustomer(event.target.value)}><option>Wilcom</option><option>WingsXP</option></select></label>}<label>Design name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Back logo - 10 inch" /></label><label>Required format<select value={format} onChange={(event) => setFormat(event.target.value)}><option>DST</option><option>EMB</option><option>NGS</option><option>EXP</option><option>PES</option><option>JEF</option><option>VP3</option><option>HUS</option><option>JPG + DST + EMB</option></select></label></div><div><label>Source artwork</label><label className="dropzone"><span className="upload-icon">↑</span><strong>{files.length ? `${files.length} file${files.length === 1 ? "" : "s"} selected` : "Choose files to upload"}</strong><small>{files.length ? files.map((file) => file.name).join(", ") : "JPG, PNG, PDF, DST, EMB, NGS, EXP, PES, JEF, VP3 or HUS"}</small><input type="file" multiple accept=".jpg,.jpeg,.png,.gif,.webp,.bmp,.tif,.tiff,.svg,.pdf,.dst,.emb,.exp,.pes,.jef,.vp3,.xxx,.hus,.ngs" onChange={(event) => setFiles(Array.from(event.target.files || []))} /></label><label>Production notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Stitch density, dimensions, thread colors..." /></label></div></div>{error && <p className="form-error">{error}</p>}<div className="form-footer"><span className="muted">Orders are visible to the assigned production team.</span><button className="button button-primary" onClick={submit}>Send order <span>→</span></button></div></section></>;
}